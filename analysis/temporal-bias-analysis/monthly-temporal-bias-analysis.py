...
def analyze_cve_distribution(nvd_data_dir: str = "../../nvd_data"):
    ...
    # Plot 2: Raw Numbers vs Percentage Comparison
    df_all["yearmonth"] = df_all["published_date"].dt.to_period("M")
    df_commits["yearmonth"] = df_commits["published_date"].dt.to_period("M")

    monthly_all = df_all["yearmonth"].value_counts().sort_index()
    monthly_commits = df_commits["yearmonth"].value_counts().sort_index()

    coverage_percentage = (monthly_commits / monthly_all * 100).fillna(0)

    # Convert period index to datetime for better plotting
    coverage_percentage.index = coverage_percentage.index.astype(str).map(
        lambda x: datetime.strptime(x, "%Y-%m")
    )
    monthly_commits.index = monthly_commits.index.astype(str).map(
        lambda x: datetime.strptime(x, "%Y-%m")
    )
    ...
    ax2.set_title("Comparison of Raw Numbers vs Percentage of CVEs with Commit Data")
    ...
    # Plot 3: Regression for number of CVEs with commit data
    dates_ordinal = [d.toordinal() for d in monthly_commits.index]
    slope, intercept, r_value, p_value, std_err = stats.linregress(
        dates_ordinal, monthly_commits.values
    )
    ...
    ax3.set_title("Regression Analysis: Number of CVEs with Commit Data Over Time")
    ax3.set_xlabel("Month")
    ax3.set_ylabel("Number of CVEs with Commit Data")
    ax3.legend()
    plt.setp(ax3.xaxis.get_majorticklabels(), rotation=45)

    # Plot 4: Regression for percentage of CVEs with commit data
    ...
    ax4.set_title("Regression Analysis: Percentage of CVEs with Commit Data Over Time")
    ...
    plt.tight_layout()
    plt.savefig("monthly_cve_temporal_distribution.png")
    plt.close()
