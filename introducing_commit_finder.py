...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("introducing_commit_finder.log"), logging.StreamHandler()]
)
...
def find_introducing_commit(repo_path: Path, fix_commit_hash: str, file_path: str) -> Optional[str]:
    """
    ...
    try:
        ...
        command = [
            ...
            fix_commit_hash + "^", # Start blaming from the commit before the fix commit
            "--",
            file_path
        ]
        ...
        if stderr:
            ...
            if "fatal: no such path" in error_message:
                logger.warning(f"File path '{file_path}' not found in repository at commit {fix_commit_hash}. It may have been introduced in the fix commit or later.")
                return None # File not found, might be introduced in fix commit or later
            elif "fatal: no commits found in range" in error_message:
                logger.warning(f"No commits found before fix commit {fix_commit_hash} for file '{file_path}'. Vulnerability might be very old or the fix commit is the first commit.")
                return None # No commits before fix commit, vulnerability might be very old
            else:
                logger.error(f"Error during git blame for {file_path} at {fix_commit_hash}: {error_message}")
        ...
        if blame_output:
            ...
            logger.warning(f"No blame output for {file_path} in {repo_path} before {fix_commit_hash}. File might be introduced in fix commit.")
            return None # No blame output, file might be introduced in fix commit

    except subprocess.TimeoutExpired:
        logger.error(f"Git blame command timed out for {file_path} in {repo_path} at {fix_commit_hash}.")
        return None
    except Exception as e:
        logger.error(f"Unexpected error during git blame for {file_path} in {repo_path} at {fix_commit_hash}: {e}")
        return None
...
def process_cve_file(cve_file_path: Path):
    ...
    try:
        with open(cve_file_path, 'r') as f:
            ...
            if not repository or not fix_commit:
                logger.warning(f"Skipping {cve_id} due to missing repository or fix commit information.")
                return

            repo_path = REPOS_DIR / repository.replace("/", "_")
            if not repo_path.exists() or not (repo_path / ".git").exists():
                logger.error(f"Repository path {repo_path} does not exist or is not a valid git repository. Skipping {cve_id}.")
                return
            ...
            if not files_in_commit:
                logger.warning(f"No files found in fix commit details for {cve_id}. Skipping file analysis.")
            else:
                logger.info(f"Processing {len(files_in_commit)} files from fix commit for {cve_id} in {repository}")
                for file_data in files_in_commit:
                    file_path = file_data['filename']
                    introducing_commit_hash = find_introducing_commit(repo_path, fix_commit, file_path)
            ...
            with open(output_file_path, 'w') as outfile:
                json.dump(cve_result, outfile, indent=4) # Write each CVE to its own file
            logger.info(f"Successfully wrote introducing commits for {cve_id} to {output_file_path}")
    ...
    logger.info(f"Successfully wrote all CVE introducing commits to {OUTPUT_DIR}") # Summary log message updated
